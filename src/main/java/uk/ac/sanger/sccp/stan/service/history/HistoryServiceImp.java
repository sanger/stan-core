...
public class HistoryServiceImp implements HistoryService {
    ...
    public History getHistoryForSamples(List<Sample> samples) {
        ...
        List<Operation> ops = opRepo.findBySampleIdIn(sampleIds);
        ...
        List<HistoryEntry> opEntries = createEntriesForOps(ops, sampleIds);
        ...
        List<HistoryEntry> entries = assembleEntries(opEntries, releaseEntries, destructionEntries);
        ...
        for (Operation op : operations) {
            ...
            for (var item : items) {
                ...
                comments.forEach(com -> {
                    if ((com.getSampleId()==null || com.getSampleId()==item.sampleId)
                            && com.getLabwareId()==null || com.getLabwareId()==item.destId) {
                        entry.addDetail(com.getComment().getText());
                    }
                });
                entries.add(entry);
            }
     ...
     */
    @SafeVarargs
    public final List<HistoryEntry> assembleEntries(Collection<HistoryEntry>... entryCollections) {
        int num = Arrays.stream(entryCollections).mapToInt(Collection::size).sum();
